...
// node.js 환경에서 돌아가기 때문에 nodejs 내장 패키지를 가져올 수 있다.
...
module.exports = {
    ...
    // mode는 개발자모드인지 production 모드인지 등을 구분
    mode: 'development',

    // 어떤 확장자명을 사용할 것인지 명시해줄 수 있다.
    // resolve의 extention에는 번들할 파일의 확장자 명시
    ...
    //  내가 앞으로 번들 할 파일들
    entry: {
        // index.jsx 기준으로 required한 것들 전부를 가져오기 때문에 최초 실행할 파일명만 명시해주면 된다.
        // 진입점 파일
        app: ['./src/index.jsx']
    },
    ...
    module: {
        ...
        rules: [{
            // 확장자가 js 이거나 jsx 일 때 (정규식) bable을 넣는다.
            test: /\.jsx?/,
            loadeer: 'babel-loader', // webpack과 babel을 연결해주는 babel 라이브러리
            // babel에 대한 옵션내용 추가
            options: {
                presets: [
                    // 옛날 브라우저에서도 환경에 맞게 실행해주는 옵션
                    ['@babel/preset-env', { // babel/present-env의 옵션 설정
                        targets: {
